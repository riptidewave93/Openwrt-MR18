...
static int ap91_dual_pci_plat_dev_init(struct pci_dev *dev)
{
	switch (PCI_SLOT(dev->devfn)) {
	case 0:
		dev->dev.platform_data = &ap9x_wmac0_data;
			       ...
			       u8 *cal_data1, u8 *mac_addr1)
{
	...
	ath79_pci_set_plat_dev_init(ap91_dual_pci_plat_dev_init);
	ath79_register_pci();

	pci_enable_ath9k_fixup(0, ap9x_wmac0_data.eeprom_data);
	pci_enable_ath9k_fixup(1, ap9x_wmac1_data.eeprom_data);
}
...
__init void ap91_dual_pci_init_simple(void)
{
